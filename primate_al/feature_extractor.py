...
class ForegroundFeatureExtractor:
    ...
    def extract_image_features(
        ...
        if pred_boxes_xyxy is not None and len(pred_boxes_xyxy) > 0:
            ...
            batch_idx = torch.zeros((len(pred_boxes_xyxy), 1), dtype=torch.float32, device=self.device)
            ...
            fg_vector = torch.mean(pooled, dim=0, keepdim=True)  # (1, 256)
        else:
            # Fallback when no boxes detected: global average pool of backbone feature map
        ...
        return fg_vector.squeeze(0).cpu().numpy()
