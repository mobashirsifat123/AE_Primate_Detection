...
\textit{1) Sanctuary Scope:} Captures originate from Nkhotakota Reserve; multi-biome African evaluations are ongoing. \textit{2) Taxa:} Focus on two sympatric primates; expanding to mixed ungulate guilds will benchmark multi-class costs. \textit{3) Costing:} In-situ stopwatch timing of field biologists will refine parameterized crowding curves.

%% ─────────────────────────────────────────────────────────────────────────────
\section{Conclusion}
We presented \textbf{AE-Primate}, resolving the dual bottlenecks of remote ecological vision: edge compute envelopes and manual bounding-box delineation costs under visual crowding. Embedding a 16-basis Gaussian RBF FastKAN bottleneck into YOLO11n ($2.45\,$M parameters), we proved that localized non-linear transformations in RKHS provide superior inductive bias for cryptic primates against dense scrub (+1.49\% mAP over CNN baselines at 32.4\,FPS on 15\,W Jetson hardware).

Benchmarking 20 active learning trajectories under strict pairwise location-disjoint evaluation ($\mathcal{S}_i \cap \mathcal{S}_j = \emptyset, \forall i \ne j$), we demonstrated that predictive uncertainty collapses under spatial covariate shift ($P_{\text{train}}(X) \neq P_{\text{test}}(X)$ with invariant $P(Y \mid X)$). AE-Primate resolves the ecological active learning trilemma: KAN Core-Set Diversity dominates out-of-distribution transfer ($0.4300$ test mAP, $p=0.011$), while crowding-penalized submodular acquisition establishes the budget Pareto frontier—saving \textbf{59.8 bounding boxes ($-5.48\%$, $p=0.0105$)} and suppressing cross-seed budgeting standard deviation by \textbf{60.3\%} (84.2\% variance reduction). All code, models, and trajectory manifests are openly available under MIT license at: \ifanonymous \url{https://anonymous.4open.science/r/AE_Primate_Detection-anon}\else \url{https://github.com/mobashirsifat123/AE_Primate_Detection}\fi.
