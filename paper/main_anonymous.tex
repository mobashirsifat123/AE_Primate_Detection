...
\begin{equation}
  ...
  \label{eq:score}
\end{equation}
with scalar exponents $\beta_1, \beta_2, \beta_3 \in \mathbb{R}_{\ge 0}$ balancing epistemic uncertainty $U(x_i) \ge 0$, diversity $D(x_i) \in [0, 2]$, and novelty $M_{\text{meta}}(x_i) \in (0, 1]$ against human annotation effort $C_{\text{annot}}(x_i) > 0$. Depending on deployment priorities, the acquisition policy supports two operational regimes: (1)~\textbf{AE-Primate (Diversity-Driven)}, configuring $(\beta_1, \beta_2, \beta_3) = (0, 1, 0)$ with $C_{\text{annot}}(x_i)=1$ to maximize topological manifold coverage for out-of-distribution generalization; and (2)~\textbf{AE-Primate (Cost-Constrained)}, deploying $(\beta_1, \beta_2, \beta_3) = (0.5, 1.0, 0.5)$ and the crowding penalty $C_{\text{annot}}(x_i)$ to minimize human labeling labor under hard budget constraints.
...
\label{sec:benchmark}
We benchmark YOLO11n-F16 against five matched edge controls and generalist baselines on $\mathcal{D}_\text{test}$ (Table~\ref{tab:model_comparison}, Figure~\ref{fig:pareto}):
\begin{itemize}
  ...
  \item \textbf{F4, F8}: FastKAN variants with $K=4$ and $K=8$ Gaussian RBF bases.
  \item \textbf{MDv6-C}: MegaDetector v6 (YOLOv9-c, 25.5\,M parameters)~\cite{beery2019efficient}, serving as an unadapted zero-shot generalist prior.
\end{itemize}
...
\midrule
Random Uniform (Passive)      & 0.4151\,$\pm$\,0.0022 & 0.4251\,$\pm$\,0.0058 & 1091.6\,$\pm$\,26.5 & Baseline (0) \\
Epistemic Uncertainty         & 0.4136\,$\pm$\,0.0035 & 0.4155\,$\pm$\,0.0084 & 1069.2\,$\pm$\,24.1 & $-$22.4 ($-$2.1\%) \\
\textbf{AE-Primate (Diversity)}  & \textbf{0.4226\,$\pm$\,0.0037}$^*$ & \textbf{0.4300\,$\pm$\,0.0045}$^*$ & 1059.5\,$\pm$\,2.4 & $-$32.1 ($-$2.9\%) \\
\textbf{AE-Primate (Cost-Aware)} & 0.4127\,$\pm$\,0.0025 & 0.4119\,$\pm$\,0.0031 & \textbf{1028.2\,$\pm$\,7.9}$^\dagger$ & \textbf{$-$63.4 ($-$5.8\%)} \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^*$Statistically significant OOD gain vs.\ passive random ($p < 0.05$, paired $t$-test).} \\
\multicolumn{5}{l}{\footnotesize $^\dagger$Statistically significant box reduction ($p = 0.008$) with 70.2\% variance reduction ($\sigma = 7.9$ vs.\ $26.5$).}
\end{tabular}%
}
\end{table}
...
\begin{enumerate}
  \item \textbf{AE-Primate (Diversity) Dominates OOD Generalization:} When configured for maximal topological manifold coverage, AE-Primate achieves the highest validation AUBC ($0.4226 \pm 0.0037$) and highest generalization on sealed test stations ($0.4300 \pm 0.0045$, Table~\ref{tab:al_benchmark}), outperforming passive random sampling with statistical significance ($p = 0.011$ on $\mathcal{D}_\text{test}$, paired $t$-test across identical seeds). By greedily maximizing dispersion across the 512-dimensional FastKAN feature manifold, diversity sampling captures rare visual archetypes across camera stations without relying on uncalibrated predictive uncertainties.
  ...
  \item \textbf{Budget Predictability via Variance Suppression:} Crucially for field deployments, AE-Primate (Cost-Aware) suppresses cross-seed box-count standard deviation from $\sigma = 26.5$ (Random) down to $\sigma = 7.9$---a \textbf{70.2\% variance reduction}. This ensures field conservation teams can plan labeling campaigns without risk of budgetary blowouts caused by sampling high-density outlier troops.
\end{enumerate}
