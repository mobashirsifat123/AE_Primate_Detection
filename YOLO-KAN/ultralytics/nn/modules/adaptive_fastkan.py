...
class C3k2_AdaptiveFastKAN(C3k2):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            AdaptiveFastKANBottleneck(self.c, self.c, shortcut=shortcut, g=g, k=(3, 3), e=1.0, max_bases=max_bases)
            for _ in range(n)
        )
...
class C3k2_FixedFastKAN(C3k2):
    ...
    def __init__(
        ...
        from ultralytics.nn.modules.fastkan import FastKANBottleneck

        super().__init__(c1, c2, n, c3k, e, g, shortcut)
        counts = list(num_bases) if isinstance(num_bases, (list, tuple)) else [int(num_bases)] * n
        if len(counts) != n or any(int(count) < 1 for count in counts):
